...
let default_cache_dir =
  Result.map (fun home ->
      "ocaml-ci-gitlab" |> Filename.concat ".cache" |> Filename.concat home) home

let default_config_dir =
  Result.map (fun home ->
      "ocaml-ci-gitlab" |> Filename.concat ".config" |> Filename.concat home) home

let cache_dir =
  Sys.getenv_opt "OCAML_CI_GITLAB_CACHE_DIR"
  |> Option.map Result.ok
  |> Option.value ~default:default_cache_dir

let config_dir =
  Sys.getenv_opt "OCAML_CI_GITLAB_CONFIG_DIR"
  |> Option.map Result.ok
